...
Original file is located at
    https://colab.research.google.com/drive/1xv4dNJEciVeK1dmDx61EObwXoVr1-qJE
"""
...
# create the policy π(s)
# SARSAがRandom要素を持っていたのと違い
# Q-Learningでは必ず最適な行動をする
...
def q_learning(action_values, exploratory_policy, target_policy, episodes, alpha=0.1, gamma=0.99):
  for episode in range(1, episodes + 1):
    ...
    while not done:
      # 探索的な行動をして結果を得る
      # off-policyなのでActionの選び方にこれまでのpolicyを使わない
      action = exploratory_policy(state)
      next_state, reward, done, _ = env.step(action)

      # 探索的に動いたあとはBestの行動をしてRewardを確認する
      next_action = target_policy(next_state)

      qsa = action_values[state][action]
      next_qsa  = action_values[next_state][next_action]
      # この計算自体はSARSAと同様
      # qsaとnext_qsaのactionの選び方だけが違う
...
#時間はかかったが関係ない部分でも最適解が得られている
# onpolicyのSARSAは探索が早いが、Bestでない場所での行動は最適ではない
plot_policy(action_values, env.render(mode='rgb_array'))
