...
Original file is located at
    https://colab.research.google.com/drive/1ge9qhjDTvGz1yMsL_QNwHhkCV-3bhnNm
"""
...
def reinforce(policy, episodes, alpha=1e-4, gamma=0.99):
  ...
  for episode in tqdm(range(1, episodes + 1)):
    ...
    while not done_b.all():
      # [0.1, 0.3, 0.5, 0.1] -> 2
      # multinomial(1) -> 確率に従って一つ選択
      # detach -> これ以降の行動が勾配計算に影響を与えぬよう

      # policy(state)で複数環境の推論を同時に実施できる
      ...
      for t, (state_t, action_t, reward_t) in reversed(list(enumerate(transitions))):
        ...
        log_probs_t = torch.log(probs_t + 1e-6) # for stable(対数計算時のゼロ割れ防止)
