...
Original file is located at
    https://colab.research.google.com/drive/1RhgL2Y9ZHHzDu4GUq79bE6o4dV3LvFUW
"""
...
# nv.observation_space -> Box([-1.2  -0.07], [0.6  0.07], (2,), float32)
# 各Stateの最大、最小値
state_dims = env.observation_space.shape[0]
num_actions = env.action_space.n
print(f"{state_dims=} {num_actions=}")

"""### Prepare the environment to work with PyTorch"""

# Pytorchで扱いやすいようEnvをWrapする
class PreprocessEnv(gym.Wrapper):
  ...
  def reset(self):
    ...
    # 　現在のstateをPytorch Tensorに変換したうえで
    # Tensorに新しい次元を追加してる
    # e.g. (n,) -> (1, n)
  ...
  def step(self, action):
    ...
    # pytorchで扱うためにそれぞれPytorch Tensorに変換する
    # unsqueezeのおかげでnext_state=tensor([[-0.4468, -0.0016]]) こうなる
    # ないとtensor([-0.4468, -0.0016])
    next_state = torch.from_numpy(next_state).unsqueeze(dim=0).float()
    reward = torch.tensor(reward).view(1, -1).float() # １行n列に変換
...
# 評価モードへの切り替え
# dropoutやnormalizationの挙動が変更され推論に最適に
...
plot_cost_to_go(env, q_network=q_network, xlabel='Car Position', ylabel='Velocity')
...
class ReplayMemory:
  ...
  def sample(self, batch_size):
    ...
    # transition => [[s, a, r, s], [s, a, r, s], [s, a, r, s]...]
    # これを学習のためにこうしたい
    # [[s, s, s, ..], [a, a, a, ...], [r, r, r, ...], [s, ,s ,s, ...]]
    batch = zip(*batch) # 上記がこれだけで変換される
