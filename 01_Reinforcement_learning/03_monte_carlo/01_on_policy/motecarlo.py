...
Original file is located at
    https://colab.research.google.com/drive/1w1VOhpMpX5j8f9hqQl1RYLe6UkQaR064
"""
...
# actionを起こしたときの結果を格納する
# そのためすべての　５ｘ５ｘ４Actionのマスが必要となる
# Q(s, a)のStateが5x5 で　a が４となる
...
#  20%でrandomに行動し、80%で最もQvalueが高い行動を選択する
def policy(state, epsilon=0.2):
  if np.random.random() < epsilon:
    return np.random.choice(4)

  av = action_values[state] # state: 5x5　-> av: (4,)

  # av.max -> bool array
  # np.flatnonzero: boolでTrueになっている１次元配列を返す。ここでは最大値を持っているものを返す
  # 最大値が重複している場合があるので、その中からRandomで返
...
def on_policy_mc_control(policy, action_values, episodes, gamma=0.99, epsilon=0.2):
  sa_returns = {} # state, actions combination

  for episodes in range(1, episodes + 1):
    state = env.reset() # 現在位置をスタートに => (0,0)
    done = False # True: 出口を見つけた
    transitions = [] # state(位置), action, reward の軌跡
    ...
    for state_t, action_t, reward_t in reversed(transitions):
      ...
      sa_returns[(state_t, action_t)].append(G) # state, actions の組み合わせにGを加算していく

      # Q valueを現在のGの価値（これまでのepsidesnの平均returns)で更新する
